  ...
  for(unsigned int k=0; k<year.size(); k++){
    for(unsigned int j=0; j<channel.size(); j++){
      for(unsigned int i=0; i<samples.size(); i++){
        TString filename;
        filename = file_dir + "Analysis_" + year.at(k) + "/chi2/" + channel.at(j) + "/uhh2.AnalysisModuleRunner.MC." + samples.at(i) + ".root";
