...
void ExtractCustomBTagSF(){
  ...
  for(unsigned int k=0; k<year.size(); k++){
    for(unsigned int j=0; j<channel.size(); j++){
      cout << year.at(k) + ": " + channel.at(j) + "..." << endl;
      for(unsigned int i=0; i<sample_names.size(); i++){
        TString filename;
        filename = file_dir + "Analysis_" + year.at(k) +"_2DBtagSF/" + channel.at(j) + "/uhh2.AnalysisModuleRunner.MC." + sample_names.at(i) + ".root";
        ...
        TFile* f;
        if(i == 0) f = new TFile("files_BTagSF/customBtagSF_" + channel.at(j) + "_" + year.at(k) + ".root", "RECREATE");
        else f = new TFile("files_BTagSF/customBtagSF_" + channel.at(j) + "_" + year.at(k) + ".root", "UPDATE");
        h_ratio->Write();
        delete f;
        ...
        c->SaveAs("Plots_BTagSF/" + year.at(k) + "/SF_" + year.at(k) + "_" + channel.at(j) +"_" + sample_names.at(i) + ".pdf");
