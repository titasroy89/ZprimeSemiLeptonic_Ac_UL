...
# Systematic name mappings for muon and electron channels
systematic_name_mapping_muon = [
    "pu", "prefiringWeight", "muonID", "muonTrigger", "muonIso", 
    "btagCferr1", "btagCferr2", "btagHf", "btagHfstats1", "btagHfstats2", 
    "btagLf", "btagLfstats1", "btagLfstats2", "ttagCorr", "ttagUncorr", "tmistag"
]

systematic_name_mapping_electron = [
    "pu", "prefiringWeight", "eleID", "eleTrigger", "eleReco", "electronIso", 
    "btagCferr1", "btagCferr2", "btagHf", "btagHfstats1", "btagHfstats2", 
    "btagLf", "btagLfstats1", "btagLfstats2", "ttagCorr", "ttagUncorr", "tmistag"
]

# Systematics for TT samples (muon and electron)
systematic_name_mapping_tt_muon = [
    "muonReco", "pu", "prefiringWeight", "muonID", "muonTrigger", "muonIso", 
    "isr", "fsr", "btagCferr1", "btagCferr2", "btagHf", "btagHfstats1", 
    "btagHfstats2", "btagLf", "btagLfstats1", "btagLfstats2", "ttagCorr", 
    "ttagUncorr", "tmistag", "jec", "jer", "murmuf", "pdf"
]

systematic_name_mapping_tt_electron = [
    "pu", "prefiringWeight", "eleID", "eleTrigger", "eleReco", "isr", 
    "fsr", "btagCferr1", "btagCferr2", "btagHf", "btagHfstats1", "btagHfstats2", 
    "btagLf", "btagLfstats1", "btagLfstats2", "ttagCorr", "ttagUncorr", 
    "tmistag", "jec", "jer", "murmuf", "pdf"
]
...
def systematics_plot(file, output_dir, sample, systematics):
    ...
    if not hist1 or not hist2 or not hist3:
        print(f"Error: Could not find one or more histograms for {sample}, {systematics}")
        return
    
    # Check for empty histograms
    if hist1.GetEntries() == 0 or hist2.GetEntries() == 0 or hist3.GetEntries() == 0:
        print(f"Warning: Empty histogram found for {sample}, {systematics}")
        return
    ...
    legend.AddEntry(hist1, f"{sample}_{systematics}Up", "l")
    legend.AddEntry(hist2, f"{sample}_{systematics}Down", "l")
    legend.AddEntry(hist3, f"{sample}", "l")
    legend.Draw()
    
    canvas.Update()
    canvas.SaveAs(os.path.join(output_dir, f"{sample}_{systematics}.png"))
...
for input_file in input_files:
    ...
    if 'muon' in input_file:
        print(f"Processing muon file: {input_file}")
        ...
        print(f"Processing electron file: {input_file}")
