    ...
    if not name in output_dict.keys():
        ...
        if not name_or_rgx in np_info_dict:
            ...
            if not name_or_rgx in master_info_dict:
                if verbosity > 0:
                    print('WARNING: syst named {name} matched to regex {name_or_rgx} not found in any of the known systematics lists.'.format(name=name, name_or_rgx=name_or_rgx))
                    print("Added to output_dict:", name, output_dict[name])
...
def check_master(s, master, local, verbosity=0):
    print("Checking systematic: {s}".format(s=s))  # Debugging print statement
    syst_not_found = True
    for syst in master.keys():
        regex = re.compile(syst)
        match = regex.match(s)
        if match and match.group(0) == s:
            local[syst] = master[syst]
            syst_not_found = False
            print("Matched systematic: {s} with {syst}".format(s=s, syst=syst))  # Debugging print statement
            break
    if syst_not_found:
        print("No match found for systematic: {s}".format(s=s))  # New debugging statement
...
def update_dict(np_dict, rule): 
    ...
    if old_name in np_dict:
        ...
        print('\nWarning: asked to rename systematic {old_name}, which is not yet in the systematics dictionary'.format(old_name=old_name))
    return np_dict
    ...
    if args.rename_dict is not None:
        ...
        for i in rename_dict:
            if cb.cp().GetParameter(i) != None:
                ...
                np_info_dict = update_dict(np_info_dict, '{i}:{rename_dict[i]}'.format(i=i, rename_dict=rename_dict[i]))
...
print("Opening global dictionary file:", args.global_dict)
with open(args.global_dict,"r") as jf:
    master_names_dict = yaml.load(jf, Loader = yaml.FullLoader)

print("Loaded master names dictionary:", master_names_dict)  # Debugging print statement
input_master = {}
...
print("Processed input master dictionary:", input_master)  # Debugging print statement
np_info = {}
...
print("Loaded systematics yaml:", np_info)  # Debugging print statement
...
datacard = args.input
print("Parsing datacard:", datacard)  # Debugging print statement
...
if do_rename:
    ...
    with open(systematics_yml, "r") as jf:
        np_info = yaml.load(jf, Loader = yaml.FullLoader)
    print("After renaming, loaded systematics yaml:", np_info)  # Debugging print statement
...
if args.make_template is not None:
    ...
    print("Systematics to be written to YAML:", systematics_dict)  # Debugging print statement

    with open(args.make_template, 'w') as jf:
        yaml.dump(systematics_dict, jf,  default_flow_style = False)
    if args.verbosity > 0:
        print('Found common systematics {systematics_dict.keys()}'.format(systematics_dict=systematics_dict.keys()))
    print("{args.make_template} created, exiting now".format(args=args))
    sys.exit()
...
if not args.skip_check:
    ...
    n_issues = len(issues)
    print('\nRun on\n  datacard: {datacard}\n  systematics file: {systematics_yml}\n  master_file: {args.global_dict}'.format(datacard=datacard, systematics_yml=systematics_yml, args=args))
    if n_issues == 0:
        print('\nSummary:  {n_checked} nuisances checked, no issues related to nuisance parameter names found.'.format(n_checked=n_checked))
    else:
        print('Summary: {n_checked} nuisances checked, nuisance check found issues related to {n_issues} nuisance parameter names'.format(n_checked=n_checked, n_issues=n_issues))
    print('\n', '#'*40, '\n')
...
if (args.do_html is not None)  or (args.do_latex is not None):
    ...
    if args.do_html is not None:
        with open(args.do_html, 'w') as f:
            ...
            f.write(styling)
            df.T.sort_values("class").to_html(f, classes="styled-table".format(args.do_html))
        print("html tables written to {args.do_html}")
    if args.do_latex is not None:
            with open(args.do_latex, 'w') as f:
                df.T.sort_values("class").to_latex(f)
            print("latex tables written to {args.do_latex}".format(args=args))
