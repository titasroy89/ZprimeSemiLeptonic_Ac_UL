    ...
    for sample in v_samples:
        ...
        if sample == "Diboson":
            for i in range(1, 101):
                nominal_hist_name = "DeltaY_reco_PDFVariations_{}_{}/DeltaY_PDF_{}".format(mass_range, lepton_flavor, i)
                nominal = inFile.Get(nominal_hist_name) 
                if not nominal:
                    print("Nominal histogram for Diboson not found.")
                    continue
                
                hist_pdfUp = nominal.Clone("{}_PDF_{}Up".format(sample, i))
                hist_pdfDown = nominal.Clone("{}_PDF_{}Down".format(sample, i))
                hist_pdfUp.Write()
                hist_pdfDown.Write()
        ...
        if sample == "TTbar":
            ...
            for quadrant in ["P_P", "P_N", "N_P", "N_N"]:
                hist_name = "DeltaY_reco_{}_750_100_muon_General/DeltaY_reco".format(quadrant)
                h_var = inFile.Get(hist_name)
                if not h_var:
            ...
            for i in range(1, 101):  
                ...
                for bin_idx in range(1, ProjX_1.GetNbinsX() + 1):
                    ...
                    deviatio2n_1 = bin_content_1 - nominal_bin_content_1
            ...
            nominal = inFile.Get("DeltaY_reco_750_1000_muon_General/DeltaY_reco")
            if not nominal:
                print("Nominal histogram for {} not found.".format(sample))
                continue

            for i in range(1, 101):
                pdf_hist = inFile.Get("DeltaY_reco_PDFVariations_750_1000_muon/DeltaY_PDF_{}".format(i))
                if pdf_hist:
