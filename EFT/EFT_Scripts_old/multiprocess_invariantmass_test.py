...
def process_file(args):
    """Process a single ROOT file to count jets and compute mttbar."""
    ...
    for ientry in range(nentries_in_file):
        ...
        try:
            ...
            if m_systweights.size() == 0:
                ...
                weight = m_systweights[0]  # Reference weight at index 0
        ...
        # Access GenParticles collection
        try:
            genparticles = getattr(tree, "GenParticles")
            n_particles = genparticles.size()
        except AttributeError:
            if debug:
                print("Warning: 'GenParticles' branch not found in event {}".format(ientry))
            continue  # Skip event if GenParticles are not available
        ...
        # Loop over GenParticles to identify top quarks
        ...
        # For each top quark, find its b quark and W boson daughters
        for top in top_quarks:
            ...
            if b_quark and w_boson:
                ...
                for decay_product in w_decay_products:
                    ...
                    if pdgId in [1, 2, 3, 4, 5]:  # Quarks
                        hadronic_decay = True
                        w_decay_quark_candidates.append(decay_product)
                    elif pdgId in [11, 13, 15]:  # e, mu, tau
                        leptonic_decay = True

                # Focus on hadronic decays to get quarks (for jet counting)
        ...
        if not partons:
            continue  # Skip event if no partons found

        # Check if slimmedGenJets are available
        try:
            genjets = getattr(tree, "slimmedGenJets")  # Updated branch name
            n_genjets = genjets.size()
        except AttributeError:
            print("Error: 'slimmedGenJets' collection not found in the ROOT file:", input_file)
            continue  # Skip event if slimmedGenJets are not available
        ...
        # Perform delta R matching between partons and jets
        ...
        for jet in jets:
            ...
            for idx, parton in enumerate(partons):
                if idx in matched_partons_indices:
                    continue  # Skip already matched partons

                # Convert GenParticle to TLorentzVector
        ...
        # Create combined_jets from matched jets
        combined_jets = [match['jet']['p4'] for match in matches]

        # Fill the histogram with the number of combined_jets
        hist_njets.Fill(len(combined_jets), weight)

        # Reconstruct Hadronic Top Quark
        # Assuming one top quark per event decays hadronically
        if len(b_quarks) >= 1 and len(w_decay_quarks) >= 2:
            # Use the first b quark and first two W decay quarks
            ...
            # Sum the four-momenta directly for accurate reconstruction
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process multiple ROOT files to count jets and compute m_ttbar.')
    parser.add_argument('input_files', nargs='+', help='List of input ROOT files')
    parser.add_argument('--output_file', default='analysis_output.root', help='Output ROOT file name')
    ...
    # If n_cores is not specified, use the number of available CPU cores
    if n_cores is None:
        n_cores = multiprocessing.cpu_count()

    # Ensure n_cores does not exceed the number of input files
    n_cores = min(n_cores, len(input_files))
    ...
    # Draw and save the number of jets histogram
    canvas_njets = ROOT.TCanvas("canvas_njets", "Number of Jets from b quark and W decay", 800, 600)
    hist_njets_total.Draw()
    canvas_njets.SaveAs("number_of_jets.png")

    # Draw and save the mttbar histogram
    canvas_mttbar = ROOT.TCanvas("canvas_mttbar", "Invariant Mass of t#bar{t}", 800, 600)
    hist_mttbar_total.Draw()
    canvas_mttbar.SaveAs("mttbar_reconstructed.png")

    print("\nProcessing complete. Histograms saved as 'number_of_jets.png', 'mttbar_reconstructed.png', and '{}'.".format(output_file_name))
