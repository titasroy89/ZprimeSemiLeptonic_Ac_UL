...
# Usage:
#   python plot_deltaY_shape.py <structure_constants.npy> <myFile.root> [--custom-wc] [--print-only] [--compare-ctGRe1] [--compare-cQj18]
...
def plot_single_wc(tree, structures, wc_index, wc_name, output_dir="plots", use_custom_wc=False):
    ...
    leg.AddEntry(hDeltaY_EFT_norm, plot_title, "lp")
    leg.Draw()
    ...
    # Save plots
    c_overlay.SaveAs(os.path.join(output_dir, "DeltaY_reco_EFT_vs_SM_normalized_overlay_%s.pdf" % filename_suffix))
...
def create_combined_ratio_plot(tree, structures, output_dir="plots", use_custom_wc=False):
    ...
    h_dummy = ROOT.TH1F("h_dummy", "", 14, -3, 3)
    h_dummy.SetTitle("")
    ...
    hDeltaY_SM = ROOT.TH1F("hDeltaY_SM_combined", "SM #DeltaY_{reco}", 14, -3, 3)
    ...
    for i, (wc_index, wc_name) in enumerate(WC_NAMES.items()):
        ...
        hDeltaY_EFT = ROOT.TH1F("hDeltaY_EFT_%s" % wc_name, "EFT #DeltaY_{reco}", 14, -3, 3)
        ...
        if use_custom_wc:
            ...
            for name, value in CUSTOM_WC_VALUES.items():
                if name in WC_INDICES:
                    wc_values[WC_INDICES[name]] = value
            legend_title = "Custom WCs"
        else:
            # Use default (single WC = 10)
            wc_values[wc_index] = 10.0
            legend_title = wc_name
        ...
        leg.AddEntry(hRatio, legend_title, "lp")
    ...
    if use_custom_wc:
        ...
        if not filename_suffix:
            filename_suffix = "SM"
        c_combined.SaveAs(os.path.join(output_dir, "DeltaY_reco_EFT_vs_SM_all_ratios_%s.pdf" % filename_suffix))
    else:
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python plot_deltaY_shape.py <structure_constants.npy> <myFile.root> [--custom-wc] [--print-only] [--compare-ctGRe1] [--compare-cQj18]")
        sys.exit(1)
    ...
    if compare_ctGRe1:
        ...
        for wc_index, wc_name in WC_NAMES.items():
            print("Processing %s (index %d)..." % (wc_name, wc_index))
            plot_single_wc(tree, structures, wc_index, wc_name, output_dir, use_custom_wc)

        
        # ratio plot
        print("\nGenerating combined ratio plot for all Wilson coefficients...")
        create_combined_ratio_plot(tree, structures, output_dir, use_custom_wc)
