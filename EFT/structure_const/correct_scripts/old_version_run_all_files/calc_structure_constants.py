...
# Usage:
#   python calc_structure_constats.py /path/to/directory/with/root/files/ [num_processes]
# Or for a single file:
#   python calc_structure_constats.py /data/dust/group/cms/zprime-uhh/Analysis_EFT_UL17/muon/workdir_Analysis_EFT_UL17_muon_semilepton_deltayreco_ttree/uhh2.AnalysisModuleRunner.MC.MC_EFT_Mttbar_0-700_UL17_2.root
...
def process_file(rootfile, output_dir, verbose=False):
    """Process a single ROOT file and save structure constants.
    
    Args:
        rootfile: Path to the ROOT file
        output_dir: Directory to save output
        verbose: Whether to print detailed information and examples
    """
    print("\nProcessing file: %s" % rootfile)
    # Generate output filename based on input filename
    basename = os.path.basename(rootfile)
    filename_without_ext = os.path.splitext(basename)[0]
    outnpy = os.path.join(output_dir, "%s_structure_constants.npy" % filename_without_ext)
    
    # Skip if output file already exists
    if os.path.exists(outnpy):
        print("Output file already exists, skipping: %s" % outnpy)
        return
    
    f = ROOT.TFile.Open(rootfile)
    if not f or f.IsZombie():
        print("Error opening file: %s" % rootfile)
        return
    ...
    print("Number of weights per event: %d" % syst0.size())
    ...
    if verbose:
        print("mg_weights shape: %s" % str(mg_weights.shape))
    ...
    print("Saved to: %s" % outnpy)

    # Print first event and examples only if verbose
    if verbose:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python calc_structure_constants.py <input_directory_or_file> [num_processes]")
        print("       If a directory is provided, all .root files in the directory will be processed")
        print("       If a file is provided, only that file will be processed")
        print("       num_processes: Number of parallel processes (default: 4)")
        sys.exit(1)
    ...
    if len(sys.argv) > 2:
        try:
            ...
            print("Warning: Invalid number of processes, using default (4)")
    
    # Create output directory for structure constants
    output_dir = "structure_constants_all_files"
    ...
    # Only print these in verbose mode or for single file
    ...
    if os.path.isdir(input_path):
        ...
        if not root_files:
            print("No ROOT files found in directory: %s" % input_path)
            sys.exit(1)
        print("Found %d ROOT files to process" % len(root_files))
        
        # Process files in parallel
        if num_processes > 1 and len(root_files) > 1:
            print("Processing files in parallel using %d processes" % num_processes)
            pool = multiprocessing.Pool(processes=num_processes)
            # First file verbose, rest quiet
            args_list = [(root_files[0], output_dir, True)]
            args_list.extend([(f, output_dir, False) for f in root_files[1:]])
            results = pool.map(process_file_wrapper, args_list)
            pool.close()
            pool.join()
            print("\nAll files processed. Results saved in: %s" % output_dir)
        else:
            # Process sequentially
            for i, rootfile in enumerate(root_files):
                # First file verbose, rest quiet
                verbose = (i == 0)
                process_file(rootfile, output_dir, verbose)
            print("\nAll files processed. Results saved in: %s" % output_dir)
    else:
        # Process a single file
        if not input_path.endswith(".root"):
            print("Error: Input file must be a ROOT file")
            sys.exit(1)
        process_file(input_path, output_dir, verbose=True)
        print("\nFile processed. Results saved in: %s" % output_dir)
