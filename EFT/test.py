...
def is_last_copy(particle):
    """Determines if a particle is the last copy before decay."""
    # Check if statusFlags method exists and use the appropriate bitmask
    # Bit 13 (1 << 13) is often used to indicate the last copy
    try:
        return particle.statusFlags() & (1 << 13) != 0
    except AttributeError:
        # If statusFlags is not available, fall back to checking number of daughters
        return particle.numberOfDaughters() == 0

def process_file(args):
    input_file = args['input_file']

    # Create histograms for last copy and reconstructed methods
    ...
    for ientry in range(nentries_in_file):
        ...
        for idx in range(n_particles):
            ...
            # Check if particle is last copy of top or antitop quark
            ...
            # Collect leptons (electron and muon)
            elif abs(pdgId) in [11, 13]:
                if pt > 30 and abs(eta) < 2.5:
                    leptons.append(particle)
            # Collect jets (light quarks and gluon)
        ...
        # Event processed flag
        event_processed = False

        # Calculate mttbar using last copy of top quarks
        ...
        # Calculate mttbar using reconstructed top quarks
        ...
        # Increment event counter if event was processed
        if event_processed:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process multiple ROOT files to produce m_ttbar histograms.')
    parser.add_argument('input_files', nargs='+', help='List of input ROOT files')
    parser.add_argument('--output_file', default='mttbar_output.root', help='Output ROOT file name')
    ...
    canvas_lastcopy.SaveAs("mttbar_lastcopy.png")
    ...
    canvas_reconstructed.SaveAs("mttbar_reconstructed.png")
