...
std::unique_ptr<TH1D> ZprimeSemiLeptonicSystematicsHists::mirror_hist_1d(const TH1D &src){
  ...
  // With 300 bins from [−1,1] this will correctly map bin i to the "-x" bin.
...
std::vector<double> ZprimeSemiLeptonicSystematicsHists::noac_mtt_edges = {0.0, 750.0, 900.0, 1100.0, 1300.0, 5000.0};
bool ZprimeSemiLeptonicSystematicsHists::noac_weights_mtt_initialized = false;

// Helper function to find mtt̄ bin index
...
Hists(ctx, dirname) {
  ...
  if(is_mc && is_tt){
    ...
    h_mtt_gen = ctx.get_handle<float>("mtt_gen");  // gen-level mtt̄ for binning
    ...
    if(use_noac_evtweights_ && !noac_gen_file_.empty()){
      ...
      if(!noac_weights_initialized || !noac_weights_mtt_initialized){
        ...
        std::vector<TH1D*> sumH_mtt;  // one per mtt̄ bin
        
        const int nmtt = (int)noac_mtt_edges.size() - 1;
        sumH_mtt.reserve(nmtt);
        for(int ib = 0; ib < nmtt; ++ib){
          TString name = TString::Format("Hgen_sum_from_tree_mttbin%d", ib);
          TH1D *h = new TH1D(name, "GEN ξ from TTree (mtt bin)", 300, -1.0, 1.0);
          h->SetDirectory(0);
          sumH_mtt.push_back(h);
        }
        ...
        if(r == 0 && gl.gl_pathc > 0){
          // Create 300-bin histogram for TTree reading (inclusive)
          sumH_inclusive = new TH1D("Hgen_sum_from_tree", "GEN histogram from TTree xi_gen branch (inclusive)", 
                          300, -1.0, 1.0);
          ...
          for(size_t i=0; i<gl.gl_pathc; ++i){
            ...
            if(tree){
              files_with_tree++;
              // Check if xi_gen and mtt_gen branches exist
              TBranch *br_xi = tree->GetBranch("xi_gen");
              TBranch *br_mtt = tree->GetBranch("mtt_gen");
              if(br_xi && br_mtt){
                files_with_branch++;
                ...
                // Build base selection string: finite xi_gen and in range [-1, 1]
                // (xi_gen == xi_gen) is false for NaN, true for finite numbers
                // (xi_gen*xi_gen < 1e10) filters out Inf values
                TString base_sel = "(xi_gen == xi_gen) && (xi_gen*xi_gen < 1e10) && (xi_gen >= -1.0) && (xi_gen <= 1.0)";
                
                // Inclusive: just xi selection (+weight)
                {
                  TString cut_expr;
                  if(has_weight) {
                    cut_expr = "weight*(" + base_sel + ")";
                  } else {
                    cut_expr = base_sel;
                  }
                  
                  TString tempName = TString::Format("tempH_incl_%zu", i);
                  TH1D *tempH = new TH1D(tempName, "temp", 300, -1.0, 1.0);
                  tempH->SetDirectory(0);
                  
                  tree->Project(tempName, "xi_gen", cut_expr, "goff");
                  sumH_inclusive->Add(tempH);
                  
                  delete tempH;
                }
                
                // mtt̄-binned histograms
                for(int ib = 0; ib < nmtt; ++ib){
                  double lo = noac_mtt_edges[ib];
                  double hi = noac_mtt_edges[ib+1];
                  
                  TString cut_expr;
                  if(has_weight){
                    cut_expr = TString::Format("weight*(%s && mtt_gen >= %f && mtt_gen < %f)",
                                               base_sel.Data(), lo, hi);
                  } else {
                    cut_expr = TString::Format("(%s && mtt_gen >= %f && mtt_gen < %f)",
                                               base_sel.Data(), lo, hi);
                  }
                  
                  TString tempName = TString::Format("tempH_mtt%d_%zu", ib, i);
                  TH1D *tempH_m = new TH1D(tempName, "temp", 300, -1.0, 1.0);
                  tempH_m->SetDirectory(0);
                  
                  tree->Project(tempName, "xi_gen", cut_expr, "goff");
                  sumH_mtt[ib]->Add(tempH_m);
                ...
                if(!br_xi) cout << "WARNING: TTree found in " << fp << " but 'xi_gen' branch not found!" << endl;
                if(!br_mtt) cout << "WARNING: TTree found in " << fp << " but 'mtt_gen' branch not found!" << endl;
              }
            } else {
              cout << "WARNING: No TTree found in " << fp << " (tried AnalysisTree)" << endl;
            }
            f->Close();
          }
          globfree(&gl);
        ...
        // Build inclusive weights
        if(sumH_inclusive && sumH_inclusive->GetNbinsX() == 300){
        ...
        // Build mtt̄-binned weights
        {
          ...
          for(const float fv : f_values){
            ...
            for(int ib = 0; ib < nmtt_built; ++ib){
              if(sumH_mtt[ib] && sumH_mtt[ib]->GetNbinsX() == 300){
                try{
                  v.push_back(build_noac_weights_from_gen(*sumH_mtt[ib], fv));
                } catch(...){
                  cout << "WARNING: Failed to build NoAC weights for f=" << fv << " mtt bin " << ib << endl;
                  v.push_back(nullptr);  // Placeholder
                }
              } else {
                v.push_back(nullptr);  // Placeholder if histogram invalid
              }
            }
...
void ZprimeSemiLeptonicSystematicsHists::fill(const Event & event){
  ...
  if(is_zprime_reconstructed_chi2 && is_mc){
    if(is_tt){
      ...
      // Get gen-level mtt̄ for binning
      double mtt_gen_evt = 0.0;
      int mtt_bin = -1;
      if(use_noac_evtweights_ && !noac_weights_mtt_map.empty()){
        try{
          mtt_gen_evt = static_cast<double>(event.get(h_mtt_gen));
          mtt_bin = find_mtt_bin(mtt_gen_evt);
        } catch(...){
          // If mtt_gen not available, mtt_bin stays -1 and we'll use inclusive weights
          mtt_bin = -1;
        }
      }
      ...
      if(use_noac_evtweights_ && !noac_weights_map.empty() && noac_weights_map.count(0.0f)){
        // Use f=0 weight for nominal (inclusive)
        w_noac_nominal = lookup_noac_weight(noac_weights_map[0.0f].get(), xi_gen_evt);
        DeltaY_xi_reco_6->Fill(deltay_xi, weight * w_noac_nominal);
        DeltaY_xi_reco_12->Fill(deltay_xi, weight * w_noac_nominal);
        DeltaY_xi_reco_18->Fill(deltay_xi, weight * w_noac_nominal);
        DeltaY_xi_reco_24->Fill(deltay_xi, weight * w_noac_nominal);
        DeltaY_xi_reco_30->Fill(deltay_xi, weight * w_noac_nominal);
        DeltaY_xi_reco_36->Fill(deltay_xi, weight * w_noac_nominal);
        DeltaY_xi_reco_50->Fill(deltay_xi, weight * w_noac_nominal);
      } else {
      ...
      // --- mtt̄-binned nominal (f=0) ---
      // Note: You'll need to book separate histograms per mtt̄ bin if you want to fill them
      // For now, we just calculate the weight but don't fill separate histograms
      double w_noac_nominal_mtt = 1.0;
      if(use_noac_evtweights_ && !noac_weights_mtt_map.empty() 
         && noac_weights_mtt_map.count(0.0f) && mtt_bin >= 0){
        const auto &vecW = noac_weights_mtt_map[0.0f];
        if(mtt_bin >= 0 && mtt_bin < (int)vecW.size() && vecW[mtt_bin]){
          w_noac_nominal_mtt = lookup_noac_weight(vecW[mtt_bin].get(), xi_gen_evt);
          // TODO: Fill mtt̄-binned histograms here if you book them
          // e.g., DeltaY_xi_reco_6_mtt[mtt_bin]->Fill(deltay_xi, weight * w_noac_nominal_mtt);
        }
      }
      ...
      if(use_noac_evtweights_ && !noac_weights_map.empty()){
        ...
        for(const float fv : f_values){
          ...
          // STEP 8: Lookup the NoAC weight for the current f value (inclusive)
          ...
          // Fill all binning schemes for this f value (inclusive)
      ...
      // --- mtt̄-binned f-scan ---
      if(use_noac_evtweights_ && !noac_weights_mtt_map.empty() && mtt_bin >= 0){
        const int bin_schemes[] = {6, 12, 18, 24, 30, 36, 50};
        static const std::map<float, std::string> f_to_suffix = {
          {-100.0f, "noacm100"}, {-12.0f, "noacm12"}, {-8.0f, "noacm8"}, {-4.0f, "noacm4"}, {-2.0f, "noacm2"},
          {-1.0f, "noacm1"}, {-0.8f, "noacm08"}, {-0.6f, "noacm06"}, {-0.4f, "noacm04"}, {-0.2f, "noacm02"},
        ...
        for(const float fv : f_values){
          auto itF = noac_weights_mtt_map.find(fv);
          if(itF == noac_weights_mtt_map.end()) continue;
          const auto &vecW = itF->second;
          if(mtt_bin < 0 || mtt_bin >= (int)vecW.size() || !vecW[mtt_bin]) continue;
          
          // Lookup mtt̄-binned weight
          const double w_f_mtt = lookup_noac_weight(vecW[mtt_bin].get(), xi_gen_evt);
          ...
          // Fill all binning schemes for this f value and mtt̄ bin
          // Naming convention: DeltaY_xi_reco_N_mtt<i>_noacX
          for(int nb : bin_schemes){
            std::string hname = "DeltaY_xi_reco_" + std::to_string(nb) + "_mtt" + std::to_string(mtt_bin) + "_" + suffix;
            auto itH = h_deltaY_xi_reco_map.find(hname);
            if(itH != h_deltaY_xi_reco_map.end()){
              itH->second->Fill(deltay_xi, weight * w_f_mtt);
            }
          }
        }
      }
