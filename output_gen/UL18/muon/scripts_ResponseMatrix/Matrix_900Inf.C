...
void Matrix_900Inf() {
    ...
    std::map<std::string, std::string> histNames = {
        {"DY_N_N_Mass_900Inf_reco_muon_General/DeltaY_reco", "h_DeltaY_N_N"}, 
        {"DY_N_P_Mass_900Inf_reco_muon_General/DeltaY_reco", "h_DeltaY_N_P"}, 
        {"DY_P_N_Mass_900Inf_reco_muon_General/DeltaY_reco", "h_DeltaY_P_N"}, 
        {"DY_P_P_Mass_900Inf_reco_muon_General/DeltaY_reco", "h_DeltaY_P_P"}
        };

    TH2D* Matrix = new TH2D("900 < Mtt", "900 < Mtt", 2, -2.5, 2.5, 2, -2.5,2.5);

    Matrix->GetXaxis()->SetTitle("#Delta |Y_{rec}|");
    Matrix->GetYaxis()->SetTitle("#Delta |Y_{gen}|");


    TFile* outFile = new TFile("900Inf.root", "RECREATE");
    ...
    TFile* openOutFile = new TFile("900Inf.root", "READ");
    ...
    TFile* outFileWrite = new TFile("900Inf.root", "UPDATE");
    outFileWrite->cd();  
    Matrix->Write();
    outFileWrite->Close();
