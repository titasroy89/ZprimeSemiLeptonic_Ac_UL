...
void Matrix_500_750() {
    ...
    std::vector<std::string> inputFiles = {"semi1.root", "other.root"};
    // std::vector<std::string> inputFiles ={"ttbar.root"};

    std::map<std::string, std::string> histNames = {
        {"DY_N_N_Mass_500_750_reco_muon_General/DeltaY_reco", "h_DeltaY_N_N"}, 
        {"DY_N_P_Mass_500_750_reco_muon_General/DeltaY_reco", "h_DeltaY_N_P"}, 
        {"DY_P_N_Mass_500_750_reco_muon_General/DeltaY_reco", "h_DeltaY_P_N"}, 
        {"DY_P_P_Mass_500_750_reco_muon_General/DeltaY_reco", "h_DeltaY_P_P"}
        };

    TH2D* Matrix = new TH2D("500< Mtt <750", "500< Mtt <750", 2, -2.5, 2.5, 2, -2.5,2.5);

    Matrix->GetXaxis()->SetTitle("#Delta |Y_{rec}|");
    Matrix->GetYaxis()->SetTitle("#Delta |Y_{gen}|");


    TFile* outFile = new TFile("500_750.root", "RECREATE");
    ...
    TFile* openOutFile = new TFile("500_750.root", "READ");
    ...
    TFile* outFileWrite = new TFile("500_750.root", "UPDATE");
    outFileWrite->cd();  
    Matrix->Write();
    outFileWrite->Close();
